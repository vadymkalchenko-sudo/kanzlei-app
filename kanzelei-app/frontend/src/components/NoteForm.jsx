...
const NoteForm = ({ note, onSubmit, onCancel }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!isDeadline) {
      ...
      delete dataToSubmit.erledigt; // Ensure erledigt is not sent if it's not a deadline
    } else if (note) {
      // If it's an existing note being turned into a deadline, initialize 'erledigt'
      dataToSubmit.erledigt = note.erledigt || false;
    } else {
      dataToSubmit.erledigt = false;
    }
